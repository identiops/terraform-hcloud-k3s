...
module "node_pool_cluster_init" {
  ...
  runcmd_first = (each.value.any.cluster_init_action.init || each.value.any.cluster_init_action.reset) ? concat([
    ...
    <<-EOT
      ...
      ## See https://github.com/rancher/charts/tree/release-v2.10/charts/system-upgrade-controller and https://github.com/rancher/system-upgrade-controller
      kubectl apply -f https://github.com/rancher/system-upgrade-controller/releases/download/latest/crd.yaml
